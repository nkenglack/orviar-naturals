...
import { Menu, X, Globe, Search, ChevronDown, Sparkles, ArrowRight } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      {showBanner && (
        <div className="bg-brand-green text-white text-xs font-semibold py-2.5 px-4 text-center relative flex items-center justify-between border-b border-green-800 shadow-inner">
          <div className="flex-1 flex items-center justify-center gap-2">
            <Sparkles size={14} className="text-brand-gold animate-pulse shrink-0" />
            <Link to="/category/all" className="hover:underline flex items-center gap-1 group">
              <span>
                {isFrench 
                  ? '🎉 Offre de Bienvenue : -20% sur TOUT LE SITE avec le code ORVIAR20 !' 
                  : '🎉 Welcome Offer: Get 20% OFF Site-Wide with code ORVIAR20 !'}
              </span>
              <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform inline" />
            </Link>
          </div>
          
          <button 
            onClick={() => setShowBanner(false)}
            className="text-white/70 hover:text-white transition-colors p-1 rounded-full hover:bg-black/10 shrink-0"
      ...
      <nav className="bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center gap-4">
            ...
            <div className="hidden md:flex items-center space-x-6 relative">
              <Link to="/" className="text-gray-700 hover:text-brand-green font-bold text-xs tracking-wider uppercase transition-colors">
              ...
              >
                <button className="flex items-center gap-1 text-gray-700 hover:text-brand-green font-bold text-xs tracking-wider uppercase py-2 transition-colors">
                  <span>{isFrench ? 'NOS PRODUITS' : 'PRODUCTS'}</span>
                  <ChevronDown size={14} />
                </button>

                {activeDropdown === 'products' && (
                  <div className="absolute top-full left-0 w-64 bg-white border border-gray-100 shadow-2xl rounded-2xl py-3 z-50">
                    <Link 
                      to="/category/all" 
                      onClick={() => setActiveDropdown(null)}
                      className="block px-4 py-2 text-xs font-extrabold text-brand-green hover:bg-green-50 uppercase tracking-wider border-b border-gray-100"
                    >
                    ...
                    {productSubCategories.map(cat => (
                      <Link
                        ...
                        onClick={() => setActiveDropdown(null)}
                        className="block px-4 py-2.5 text-xs font-semibold text-gray-700 hover:text-brand-green hover:bg-gray-50 transition-colors"
                      >
              ...
              >
                ...
                {activeDropdown === 'benefits' && (
                  <div className="absolute top-full left-0 w-72 bg-white border border-gray-100 shadow-2xl rounded-2xl py-3 z-50">
              ...
              <Link to="/blog" className="text-gray-700 hover:text-brand-green font-bold text-xs tracking-wider uppercase transition-colors">
                BLOG
              </Link>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-4">
              ...
              <Link to="/distributor" className="bg-brand-green text-white px-5 py-2.5 rounded-full text-xs font-bold hover:bg-green-800 transition-all shadow-md shrink-0 uppercase tracking-wider">
                {t('nav.distributor')}
              </Link>
            </div>
        ...
        {/* Mobile Menu Drawer */}
        {isOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 shadow-2xl absolute w-full left-0 px-4 pt-4 pb-8 space-y-4 max-h-[85vh] overflow-y-auto z-50">
            <form onSubmit={handleSearchSubmit} className="relative w-full">
              ...
              <input 
                ...
                value={navSearch}
                onChange={(e) => setNavSearch(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 bg-gray-100 rounded-full text-xs text-gray-900 focus:outline-none"
              />
            </form>

            <Link to="/" onClick={() => setIsOpen(false)} className="block py-2 text-sm font-extrabold text-gray-800 uppercase">
              {isFrench ? 'ACCUEIL' : 'HOME'}
            </Link>
              ...
              <div className="pl-3 space-y-2 border-l-2 border-green-100">
                {productSubCategories.map(cat => (
                  <Link
                    ...
                    className="block text-xs font-medium text-gray-600 py-1"
                  >
                    {isFrench ? cat.fr : cat.en}
                  </Link>
                ))}
            ...
            <Link to="/blog" onClick={() => setIsOpen(false)} className="block py-2 text-sm font-extrabold text-gray-800 uppercase">
              BLOG
            </Link>

            <Link to="/distributor" onClick={() => setIsOpen(false)} className="block text-center w-full bg-brand-green text-white py-3 rounded-full font-bold text-xs uppercase tracking-wider">
              {t('nav.distributor')}
            </Link>
          </div>
        )}
